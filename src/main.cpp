...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 15); // Kushcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kushcoin Signed Message:\n";
...
{
    // Kushcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	int64 nSubsidy = 500 * COIN;
        ...
        if(nHeight < 10000)
        {
                nSubsidy *= 3; // 1500 per block
        }
        else if(nHeight >10000 && nHeight < 20001)
        {
                nSubsidy *= 10; // 5,000 per block
        }
        else if(nHeight > 20000 && nHeight < 50001)
        {
                nSubsidy *= 20; // 10,000 per block
        }
        else if(nHeight >50000 && nHeight <150001)
        {
                nSubsidy *= 2; // 1,000 per block
        }
...
static const int64 nTargetTimespan = 7 * 60 * 60; // Kushcoin: 7 hours
static const int64 nTargetSpacing = 60; // Kushcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kushcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kushcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kushcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Jan 09 2014 Colorado Made 5M in 5 Days Off Pot. Problem - It is Illegal to Put the Money in Federal Banks";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kushcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KushcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KushcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KushcoinMiner(CWallet *pwallet)
{
    printf("KushcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kushcoin-miner");
    ...
    try { loop {
        ...
        printf("Running KushcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KushcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KushcoinMiner, pwallet));
}
